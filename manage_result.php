...
$user="SELECT * FROM student_result WHERE sid='$sid' AND assessment='$assessment' AND course='$course'";
$query = mysqli_query($conn, $user);
  ...
  if(mysqli_num_rows($query) > 0 ) { //check if there is already an entry for that username
    ...
    $sql="INSERT INTO student_result (`sid`, `assessment`, `course`, `totalmarks`, `gainmarks`, `percentage`) VALUES ('$sid', '$assessment', '$course', '$totalmarks', '$gainmarks', '$percentage')";
                ...
                <table class="table table-bordered text-center" id="dataTable" width="100%" cellspacing="0">
                  ...
                  <tbody>
                    <tr>
                      ...
                      <td><a href="delete_student_result.php?sid=<?php echo $row['sid']; ?>&assessment=<?php echo $row['assessment']; ?>" class="btn btn-danger btn-sm text-light">Delete</a></td>
                      <td><a href="student_update_result.php?sid=<?php echo $row['sid']; ?>&assessment=<?php echo $row['assessment']; ?>&course=<?php echo $row['course']; ?>" class="btn btn-primary btn-sm text-light">Update</a></td>
          ...
          <div class="modal-body" style="background-image: linear-gradient(#000066,#ffffff);">
            ...
            <section class="logsection">
               <div class="logform">
                  ...
                  <label for=""><b>SId</b></label><br>
                  <?php
                    $sql = "SELECT sid FROM student_course WHERE course = '" .$_GET['course']. "'";
                    $result = $conn->query($sql);
                        echo "<select name='sid' class='form-select w-75' aria-label='Default select example'>";
                 ...
                 <?php
                    $sql = "SELECT DISTINCT assessment FROM quiz";
                    $result = $conn->query($sql);
